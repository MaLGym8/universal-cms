...
function Show_Gallery()
{
    ...
    if (isset($_POST["set_position"])) {
        $items = $_POST['portf'];
        mysql_query("UPDATE images_content SET public=0 WHERE `cat`=2");
        if ($items != "") {
            foreach ($items as $id) {
                mysql_query("UPDATE images_content SET public=1 WHERE id=$id");
            }
        }
        echo '<script>window.location="index.php?page=show_gallery";</script>';
    }
    ...
    if (!isset($id)) {
        ...
        <header><h3>Редактирование галереи <span class="header-small">(перетаскивая, можно менять позиции)</span></h3></header>
        <div class="module_content">

        <form method="post" action="index.php?page=show_gallery">

            <div align="center"><input onclick="window.location='index.php?page=show_gallery_add'" type="button" class="add-image-button" value="Добавить изображение"/></div>

                <ul id="list1">
                    <?
                    $result = mysql_query("SELECT * FROM images_content WHERE `cat`=2 ORDER by sort");
                    if (!$result) {
	...
	<a href="index.php?page=show_gallery&id=' . $myrow["id"] . '">
...
<header><h3>Редактирование изображения галереи</h3></header>
<div class='module_content'>";

        echo '
<form enctype="multipart/form-data" name="form"  id="forma" method="post" action="includes/gallery/index_update.php">
 ...
 <p>
        ...
        echo "<div class='delete-image'><form name='delport' action='includes/gallery/index_drop.php' method='post'><input name='delport' type='hidden' value='$myrow[id]'><input name='submit' type='submit' class='delete-image-but' value='X'></form>";
...
function  Show_Gallery_Add()
{
?>
    <article class="module width_full">
    <header><h3>Добавление изображения галереи</h3></header>
    <div class="module_content">


        <form enctype="multipart/form-data" name="form" id="forma" method="post" action="includes/gallery/add.php">
