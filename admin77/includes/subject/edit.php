...
{
    $result = mysql_fetch_array(mysql_query("SELECT * FROM `subject` WHERE id=$id"));

    echo '
    <article class="module width_full">
    <header><h3>Редактирование тематики</h3></header>
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        Наименование:<br/>
        <input type="text" name="namesub" value="'.$result["subject_name"].'"/><br />
        Подкатегория:<br/>
    ...
    echo'
        ...
        <input type="submit" name="edit_subject" value="Изменить" >
    </form>
...
if(isset($_POST["edit_subject"]))
{
    ...
    mysql_query("UPDATE `subject` SET `title`='$title',`meta_d`='$meta_d',`meta_k`='$meta_k',`desc`='$desc',`image`='$thumb',`parent`='$parent',`position`='$position', `url`='$url',`menu`='$menu',`h1`='$h1',`subject_name`='$subject_name' WHERE id='$id'");

    $_SESSION["message"] = "Тематика изменена";
