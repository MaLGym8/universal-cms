<?php
//Просмотр категорий
function Show_Catalog_Gift()
{
    $tableName = "catalog_gift";
    $count = mysql_fetch_row(mysql_query("SELECT COUNT(*) FROM `$tableName`"));


    echo '
    <article class="module width_3_quarter">
		<header><h3 class="tabs_involved">Категории (' . $count[0] . ')  <a href="index.php?page=add_catalog_gift" class="event-button">Добавить подарок</a></h3></header>
    ...
    for ($i = 0; $i < $num; $i++) {
        if ($postrow[$i]) {
            ...
            <td>' . $postrow[$i]["desc"] . '</td>
            <td><img src="../' . $postrow[$i]["image"] . '"/></td>

            <td align="center"><a  href="index.php?page=edit_catalog_gift&id=' . $postrow[$i]["id"] . '"><img style="margin-right:7px;" src="images/icn_edit.png" title="Редактировать"/></a></td>
			<td align="center"><span class="del-punkt"><a onclick="return onclick_delete()" href="?delete_catalog_gift=' . $postrow[$i]["id"] . '"><img src="images/icon_delete.gif"  title="Удалить"/></a></span></td>
            </tr>';
...
if(isset($_GET["delete_catalog_gift"]))
{
    $id = $_GET["delete_catalog_gift"];

    @removeDirRec("../files/gift/".$id);

    mysql_query("DELETE FROM catalog_gift WHERE id=$id");
