...
function Show_Sert()
{
    ...
    if (isset($_POST["set_position"])) {
        $items = $_POST['portf'];
        mysql_query("UPDATE images_content SET public=0 WHERE `cat`=4");
        if ($items != "") {
            foreach ($items as $id) {
                mysql_query("UPDATE images_content SET public=1 WHERE id=$id");
            }
        }
        echo '<script>window.location="index.php?page=show_sert";</script>';
    }
    ...
    if (!isset($id)) {
        ...
        <header><h3>Редактирование сертификатов <span class="header-small">(перетаскивая, можно менять позиции)</span></h3></header>
        <div class="module_content">

        <form method="post" action="index.php?page=show_sert">
        
            <div align="center"><input onclick="window.location='index.php?page=show_sert_add'" type="button" class="add-image-button" value="Добавить сертификат"/></div>
            
                <ul id="list1">
                    <?
                    $result = mysql_query("SELECT * FROM images_content WHERE `cat`=4 ORDER by sort");
                    if (!$result) {
	...
	<a href="index.php?page=show_sert&id=' . $myrow["id"] . '">
...
<header><h3>Редактирование сертификата</h3></header>
<div class='module_content'>";
...
function  Show_Sert_Add()
{
?>
    <article class="module width_full">
    <header><h3>Добавление сертификата</h3></header>
    <div class="module_content">
