...
$num=sitequantity1;
$result = $db->read_all("SELECT COUNT(*) as rec FROM `news` WHERE `type`=2 AND `public`=1");
$posts = $result[0]["rec"];
...
$newsQ = $db->read_all("SELECT `id`,`url`,`title`,`text`,`date_add`,`image`,`public` FROM `news` WHERE `type`=2  AND `public`=1 ORDER by `date_add` DESC LIMIT $start, $num");
    ...
    foreach($newsQ as $newsR)
    {
        echo'<div class="item-news">
                                <div class="item-news-p">
                                    ...
                                    <div class="photo"><a href="'.$base_url.'articles/'.$newsR["url"].'"><img src="'.$base_url.$newsR["image"].'"/></a></div>
