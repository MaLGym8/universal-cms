...
function Show_Cats()
{
    $tableName = "catalog_cat";
    $count = mysql_fetch_row(mysql_query("SELECT COUNT(*) FROM `$tableName`"));


    echo '
    <article class="module width_3_quarter">
		<header><h3 class="tabs_involved">Категории (' . $count[0] . ')  <a href="index.php?page=add_cats" class="event-button">Добавить категорию</a></h3></header>
    ...
    for ($i = 0; $i < $num; $i++) {
        if ($postrow[$i]) {
            ...
            <td><a  href="index.php?page=edit_cats&id=' . $postrow[$i]["id"] . '"><b>' . $postrow[$i]["title"] . '</b></a></td>
            <td class="urloverflow150"><a  href="index.php?page=edit_cats&id=' . $postrow[$i]["id"] . '"><b>' . $postrow[$i]["url"] . '</b></a></td>
            <td>' . $postrow[$i]["menu"] . '</td>
            <td>' . $postrow[$i]["meta_d"] . '</td>
            <td>' . $postrow[$i]["meta_k"] . '</td>
            <td>' . $postrow[$i]["h1"] . '</td>
            <td>' . CutTextNews($postrow[$i]["desc"],200) . '</td>
            <td align="center"><a  href="index.php?page=edit_cats&id=' . $postrow[$i]["id"] . '"><img style="margin-right:7px;" src="images/icn_edit.png" title="Редактировать"/></a></td>
			<td align="center"><span class="del-punkt"><a onclick="return onclick_delete()" href="?delete_cats=' . $postrow[$i]["id"] . '"><img src="images/icon_delete.gif"  title="Удалить"/></a></span></td>
            </tr>';

            $QC1 = mysql_query("SELECT * FROM `catalog_cat` WHERE `parent`='".$postrow[$i]["id"]."'");
            ...
            <td >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a  href="index.php?page=edit_cats&id=' . $RC1["id"] . '">' . $RC1["title"] . '</a></td>  
            <td class="urloverflow150">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a  href="index.php?page=edit_cats&id=' . $RC1["id"] . '">' . $RC1["url"] . '</a></td>
            <td>' . $RC1["menu"] . '</td>
            <td>' . $RC1["meta_d"] . '</td>
            <td>' . $RC1["meta_k"] . '</td>
            <td>' . $RC1["h1"] . '</td>
            <td>' . CutTextNews($RC1["desc"],200) . '</td>
            <td align="center"><a  href="index.php?page=edit_cats&id=' . $RC1["id"] . '"><img style="margin-right:7px;" src="images/icn_edit.png" title="Редактировать"/></a></td>
			<td align="center"><span class="del-punkt"><a onclick="return onclick_delete()" href="?delete_cats=' . $RC1["id"] . '"><img src="images/icon_delete.gif"  title="Удалить"/></a></span></td>
            </tr>';



                    $QC2 = mysql_query("SELECT * FROM `catalog_cat` WHERE `parent`='".$RC1["id"]."'");
            ...
            <td >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a  href="index.php?page=edit_cats&id=' . $RC2["id"] . '"><i>' . $RC2["title"] . '</i></a></td>            <td >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a  href="index.php?page=edit_cats&id=' . $RC2["id"] . '"><i>' . $RC2["url"] . '</i></a></td>
            <td>' . $RC2["menu"] . '</td>
            <td>' . $RC2["meta_d"] . '</td>
            <td>' . $RC2["meta_k"] . '</td>
            <td>' . $RC2["h1"] . '</td>
            <td>' .CutTextNews( $RC2["desc"],200) . '</td>
            <td align="center"><a  href="index.php?page=edit_cats&id=' . $RC2["id"] . '"><img style="margin-right:7px;" src="images/icn_edit.png" title="Редактировать"/></a></td>
			<td align="center"><span class="del-punkt"><a onclick="return onclick_delete()" href="?delete_cats=' . $RC2["id"] . '"><img src="images/icon_delete.gif"  title="Удалить"/></a></span></td>
            </tr>';
...
{
    $id = $_GET["delete_cats"];
    $resultCat = mysql_fetch_array(mysql_query("SELECT * FROM `catalog_cat` WHERE id=$id"));
    @unlink("../".$resultCat["image"]);
    @unlink("../".str_replace("small","big",$resultCat["image"]));
    mysql_query("DELETE FROM `catalog_cat` WHERE id=$id");
