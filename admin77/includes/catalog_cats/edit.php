...
function Edit_Cats($id = NULL)
{
    $result = mysql_fetch_array(mysql_query("SELECT * FROM `catalog_cat` WHERE id=$id"));
    ...
    echo'
        ...
        <input type="submit" name="edit_cats" value="Изменить" >
    </form>
...
if(isset($_POST["edit_cats"]))
{
    ...
    mysql_query("UPDATE `catalog_cat` SET `title`='$title',`meta_d`='$meta_d',`meta_k`='$meta_k',`desc`='$desc',`image`='$thumb',`parent`='$parent',`position`='$position', `url`='$url',`menu`='$menu',`h1`='$h1' WHERE id='$id'");
