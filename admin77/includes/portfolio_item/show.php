...
echo "<div style='float:left;width: 100%;font-size: 16px;'>Найдено работ: <b>$COUNTALL</b></div>";
	...
	if ($myrow["site_href"]) echo '<span class="link"><a href="http://' . $myrow["site_href"] . '" target="_blank">http://' . $myrow["site_href"] . '</a></span><br />';
		  ...
		  <p>
                    ...
                    <p>
                        <label>Ссылка на сайт<br>
                            <input value="'.$myrow["site_href"].'" type="text" name="site_href" style="width:70%;">
                        </label>
                    </p>
...
if ($myrow['public'] == 1
            ...
            if($myrow["background"])
            {
                echo "<br/><img class='delete_img' type='bg_portfolio' alt='".$myrow["background"]."'  src='../".$myrow["background"]."' style='width:100px;'/>";
            }

            echo'<br/></p>
                <p>
                Фон (Цвет):<br/> 
                <input type="text" value="'.$myrow["background_color"].'" name="background_color"/><br/></p>
                <p>
                Фон (Цвет текста):<br/>
                <input type="text" value="'.$myrow["background_text"].'" name="background_text"/><br/></p>
...
if(isset($_POST["edit_portfolio"]))
{
    ...
    mysql_query("UPDATE `portfolio_item` SET `title`='$title', `meta_d`='$meta_d',`meta_k`='$meta_k', `url`='$url',`name`='$name', `h1`='$h1', `text`='$text', `public_text`='$public_text', `site_href`='$site_href', `iframe`='$iframe', `sort`='$position', `public`='$public', `portfolio_cat`='$cat',`background_color`='$background_color', `background_text`='$background_text' WHERE `id`='$id'");
