...
function Show_Catalog()
{
    ...
    <article class="module width_3_quarter">
		<header><h3 class="tabs_involved">Каталог (' . $count[0] . ')  <a href="index.php?page=add_catalog" class="event-button">Добавить товар</a></h3></header>
		<div class="tab_container">
			...
			<thead>
				<tr>
                    ...
                    <th>URL</th>
                    <th>Категория</th>';
    if(CATALOGTYPE==1){echo'<th>Стоимость</th>';}
    echo'<th>Тип</th>
    				<th></th>
					<th></th>
				</tr>
    ...
    $query = mysql_query("SELECT * FROM `$tableName` ORDER BY coast,id LIMIT $start, $num");
...
$CAT = mysql_fetch_array(mysql_query("SELECT * FROM `catalog_cat` WHERE `id`='".$postrow[$i]["cat"] ."'"));

            if($postrow[$i]["name"])
                $title = $postrow[$i]["name"];
            else
                $title = $postrow[$i]["title"];
            echo '<tr>
			<td>' . $postrow[$i]["id"] . '</td>
            <td><img width="70" src="../' . $postrow[$i]["image"] . '"/></td>
            <td><a  href="index.php?page=edit_catalog&id=' . $postrow[$i]["id"] . '"><b>' . $title . '</b></a></td>
            <td class="urloverflow150"><a  href="index.php?page=edit_catalog&id=' . $postrow[$i]["id"] . '"><b>' . $postrow[$i]["url"] . '</b></a></td>
            <td>' .$CAT["title"] . '</td>
            
            ';
    if(CATALOGTYPE==1){echo'<td>' . $postrow[$i]["coast"] . '</td>';}
    echo'<td>' . $public1 . '</td>
            <td align="center"><a  href="index.php?page=edit_catalog&id=' . $postrow[$i]["id"] . '"><img style="margin-right:7px;" src="images/icn_edit.png" title="Редактировать"/></a></td>
			<td align="center"><span class="del-punkt"><a onclick="return onclick_delete()" href="?delete_catalog=' . $postrow[$i]["id"] . '"><img src="images/icon_delete.gif"  title="Удалить"/></a></span></td>
            </tr>';
...
{
    $id = $_GET["delete_catalog"];

    @removeDirRec("../files/catalog/".$id);
