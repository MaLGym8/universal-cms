...
function Show_Sliders()
{
    ...
    if (isset($_POST["set_position"])) {
        $items = $_POST['portf'];
        mysql_query("UPDATE images_content SET public=0 WHERE `cat`=1");
        if ($items != "") {
            foreach ($items as $id) {
                mysql_query("UPDATE images_content SET public=1 WHERE id=$id");
            }
        }
        echo '<script>window.location="index.php?page=show_sliders";</script>';
    }
    ...
    if (!isset($id)) {
        ...
        <header><h3>Редактирование слайдера <span class="header-small">(перетаскивая, можно менять позиции)</span></h3></header>
        <div class="module_content">

        <form method="post" action="index.php?page=show_sliders">
            
            <div align="center"><input onclick="window.location='index.php?page=show_sliders_add'" type="button" class="add-image-button" value="Добавить слайд"/></div>
            
                <ul id="list1">
                    <?
                    $result = mysql_query("SELECT * FROM images_content WHERE `cat`=1 ORDER by sort");
                    if (!$result) {
	...
	<li style="float:left;" id="position_' . $myrow["id"] . '" class="li-card"><div ' . $opacity . ' class="card">
	<a href="index.php?page=show_sliders&id=' . $myrow["id"] . '">
	<div class="img"><img style="max-width:250px;"  src="../' . $myrow["fullimg"] . '"></div></a>
...
<header><h3>Редактирование слайда</h3></header>
<div class='module_content'>";
...
function  Show_Sliders_Add()
{
?>
    <article class="module width_full">
    <header><h3>Добавление слайда</h3></header>
    <div class="module_content">
