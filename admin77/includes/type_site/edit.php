...
function Edit_Type_Site($id = NULL)
{
    $result = mysql_fetch_array(mysql_query("SELECT * FROM `type_site` WHERE id=$id"));

    echo '
    <article class="module width_full">
    <header><h3>Редактирование типа сайта</h3></header>
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        Наименование:<br/>
        <input type="text" name="type_site" value="'.$result["type_site"].'"/><br />
        Подкатегория:<br/>
        <select name="parent"><option value="0">Выберите подкатегорию</option>';

    $queryC1 = mysql_query("SELECT * FROM `type_site`  WHERE parent=0  ORDER by title ASC");
    ...
    do{
        if($resultC1["id"]) {
            ...
            $queryC2 = mysql_query("SELECT * FROM `type_site`  WHERE parent=". $resultC1["id"] ."  ORDER by title ASC");
            ...
            do{
                if($resultC2["id"]) {
                    echo '<option value="' . $resultC2["id"] . '"'; if($result["parent"]== $resultC2["id"])echo'selected'; echo'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-' . $resultC2["title"]
                        . '</option>';
                    $queryC3 = mysql_query("SELECT * FROM `type_site`  WHERE parent=". $resultC2["id"] ."  ORDER by title ASC");
    ...
    echo'
        ...
        <input type="submit" name="edit_type_site" value="Изменить" >
    </form>
...
if(isset($_POST["edit_type_site"]))
{
    ...
    mysql_query("UPDATE `type_site` SET `title`='$title',`meta_d`='$meta_d',`meta_k`='$meta_k',`desc`='$desc',`image`='$thumb',`parent`='$parent',`position`='$position', `url`='$url',`menu`='$menu',`h1`='$h1',`type_site`='$type_site' WHERE id='$id'");
