...
{
    $tableName = "options_catalog";
    $count = mysql_fetch_row(mysql_query("SELECT COUNT(*) FROM `$tableName`"));


    echo '
    <article class="module width_3_quarter">
		<header><h3 class="tabs_involved">Опции (' . $count[0] . ')  <a href="index.php?page=add_options" class="event-button">Добавить опцию</a></h3></header>
    ...
    for ($i = 0; $i < $num; $i++) {
        if ($postrow[$i]) {
            ...
            {
                case "1": $public1 = "<b style='color:#008c2e;'>Опубликовано</b>";break;
                case "2": $public1 = "<b style='color:#8c0000;'>Скрыто</b>";break;
            }

            switch($postrow[$i]["tofilter"])
            {
                case "1": $tofilter = "<b style='color:#008c2e;'>Опубликовано</b>";break;
                case "2": $tofilter = "<b style='color:#8c0000;'>Скрыто</b>";break;
            }
            ...
            echo '<tr>
			<td>' . $postrow[$i]["id"] . '</td>
            <td><a  href="index.php?page=edit_options&id=' . $postrow[$i]["id"] . '"><b>' . $postrow[$i]["name"] . '</b></a></td>
            ...
            <td>' . $public1 . '</td>
            <td>' . $tofilter . '</td>
            <td align="center"><a  href="index.php?page=edit_options&id=' . $postrow[$i]["id"] . '"><img style="margin-right:7px;" src="images/icn_edit.png" title="Редактировать"/></a></td>
			<td align="center"><span class="del-punkt"><a onclick="return onclick_delete()" href="?delete_options=' . $postrow[$i]["id"] . '"><img src="images/icon_delete.gif"  title="Удалить"/></a></span></td>
            </tr>';
...
{
    $id = $_GET["delete_options"];

    @removeDirRec("../files/options/".$id);

    mysql_query("DELETE FROM options_catalog WHERE id=$id");
    mysql_query("DELETE FROM options_values WHERE option_id=$id");
    mysql_query("DELETE FROM options_products WHERE option_id=$id");
    $_SESSION["message"] = "Опция удалена";
